...
void QFtpDTP::writeData()
{
    ...
    if (is_ba) {
        ...
        callWriteData = false;
        const qint64 blockSize = 128*1024;
        char buf[128*1024];
...
public:
    ...
    inline QFtpPrivate(QFtp *owner) : close_waitForStateChange(false),
        transferMode(QFtp::Passive), error(QFtp::NoError), q_ptr(owner)
    {
        setState(QFtp::Unconnected);
    }
    ...
    QFtp::State state();
    void setState(QFtp::State newstate);
...
private:
    //begin
    QReadWriteLock m_stateLock;//多线程访问状态需要上锁
    static QFtp::State m_state;//多线程操作需要记录唯一的状态
    //end
};

QFtp::State QFtpPrivate::m_state;
...
void QFtpPrivate::setState(QFtp::State newstate)
{
...
void QFtp::setState(State newstate) const
{
    d->setState(newstate);
}
...
{
    setState(QFtp::State(connectState));
