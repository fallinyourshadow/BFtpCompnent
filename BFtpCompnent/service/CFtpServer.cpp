...
{
    pFirstUser = pLastUser = nullptr;
    pFirstClient = pLastClient = nullptr;
    ...
    _OnUserEventCb = nullptr;
    _OnClientEventCb = nullptr;
    _OnServerEventCb = nullptr;
#endif
        ...
        if( IsListening() ) {
            ...
            if( IsAccepting() ) {
                ...
                if( !pthread_cancel( AcceptingThreadID ) )
                    pthread_join( AcceptingThreadID, nullptr );
#endif
            }
            bIsListening = false;
            FtpServerLock.Leave();
...
#endif
{
    ...
    for( ;; ) {
        ...
        if( select( ListeningSock + 1, &fdRead, nullptr, nullptr, nullptr ) == SOCKET_ERROR )
            break;
...
char* CFtpServer::CClientEntry::BuildPath( char* pszAskedPath, char **pszVirtualPath /* = nullptr */ )
{
    if( !pUser )
        return nullptr; // Client is not logged-in.
        ...
        if( pszAskedPath ) {
            if( pszAskedPath[ 0 ] == '/' ) {
                ...
                if( strlen( pszAskedPath ) <= MAX_PATH ) {
                    strcpy_s( pszBuffer, MAX_PATH + 4, pszAskedPath);
                } else
                    bErr = true;
            } else {
                if( snprintf( pszBuffer, MAX_PATH + 4, "%s/%s",
            ...
            strcpy_s( pszBuffer, MAX_PATH + 4 , GetWorkingDirectory() );
...
bool CFtpServer::SimplifyPath( char *pszPath )
{
    ...
    for( a = pszPath; *a != '\0'; ++a ) // Replace all '\' by a '/'
        if( *a == '\\' ) *a = '/';
    while( ( a = strstr( pszPath, "//") ) != nullptr ) // Replace all "//" by "/"
    ...
    // Remove the ending '/'
    qint64 iPathLen = strlen( pszPath );
...
CFtpServer::CUserEntry *CFtpServer::AddUser( const char *pszLogin, const char *pszPass, const char *pszStartDir )
{
            ...
            && ( !pszPass || strlen( pszPass ) <= MaxPasswordLen )	)
    {
        ...
        UserListLock.Enter();
        {
            if( SearchUserFromLogin( pszLogin ) != nullptr )
        ...
        if( pUser ) {
            ...
            strcpy_s( pUser->szLogin, sizeof(pUser->szLogin), pszLogin );
            strcpy_s( pUser->szStartDirectory, sizeof(pUser->szStartDirectory), pszSDEx ); free( pszSDEx );
            if( pszPass ) strcpy_s( pUser->szPassword, sizeof(pUser->szPassword), pszPass );
            ...
            OnUserEventCb( NEW_USER, pUser, nullptr );
...
{
    if( pszLogin ) {
        CFtpServer::CUserEntry *pSearchUser = nullptr;
...
CFtpServer::CClientEntry::CClientEntry()
{
    ...
    pszCmdArg = psNextCmd = nullptr;
    pPrevClient = pNextClient = nullptr;
...
bool CFtpServer::CClientEntry::ReceiveLine()
{
    ...
    for( ;; ) {
        ...
        ptv = nullptr; tv.tv_sec = 0; tv.tv_usec = 0;
        ...
        if( select( CtrlSock + 1, &fdRead, nullptr, nullptr, ptv ) == SOCKET_ERROR )
            break;
...
bool CFtpServer::CClientEntry::OpenDataConnection( int nCmd )
{
                                                   ...
                                                   ( nCmd == CMD_STOR ) ? SO_RCVBUF : SO_SNDBUF, (char*)&iBufSize, sizeof(int) ) )
    {
        ...
        if( eDataConnection == PASV ) {
            ...
            if( select( DataSock + 1, &fdRead, nullptr, nullptr, &tv ) > 0 && FD_ISSET( DataSock, &fdRead ) )
            {
...
{
    int nRet;
    pTransfer->zStream.zfree = Z_nullptr;
    pTransfer->zStream.zalloc = Z_nullptr;
    pTransfer->zStream.opaque = Z_nullptr;
                                               ...
                                               time_t mtime, const char* pszName, bool opt_F )
{
    ...
    if( time(nullptr) - mtime > 180 * 24 * 60 * 60 ) {
