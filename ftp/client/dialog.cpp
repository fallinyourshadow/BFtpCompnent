    ...
    ui(new Ui::Dialog)
{
   ...
   // this->resize(900,500);
    ...
    m_loginState = false;
    m_pFtp = new QFtp;
    QObject::connect(m_pFtp,SIGNAL(stateChanged(int)),this,SLOT(stateChangedSlot(int)));
...
void Dialog::doubleclickItemSlot(QTreeWidgetItem *treeWgtItem, int)
{
    ...
    m_pFtp->cd(stringToFtp(treeWgtItem->text(0)));
}
...
void Dialog::ListInfoSlot(const QUrlInfo &info)
{
    ...
    if ( ftpToString(info.name()) == "." || ftpToString(info.name()) == "..") {
        return;
    }
    ...
    item->setText(0, ftpToString(info.name()));
...
void Dialog::lineDdieEnterSlot()
{
    ...
    m_pFtp->cd(stringToFtp(ui->lineEditPath->text().trimmed()));
}
...
void Dialog::dropEvent(QDropEvent *event)
{
    ...
    foreach (QUrl url, urlList) {
        ...
        if ( info.isDir()) {
            ...
            uploadFile(QStringList(filePath));
        }
    }
    event->acceptProposedAction();
}
...
void Dialog::uploadFile(QStringList &fileList)
{
    ...
    foreach (QString filePath, fileList) {
        ...
        thread->sendFile(filePath,stringToFtp(m_currentDir),
                         stringToFtp(fileInfo.fileName()),m_progress);
        m_threadNumber++;
        m_threadMutex.unlock();
    }
}
...
void Dialog::renameSlot(QTreeWidgetItem *item, int)
{
    ...
    m_pFtp->rename(stringToFtp(m_oldName),
                   stringToFtp(m_oldItem->text(0)));
    m_oldName.clear();
    m_oldItem = NULL;
}
...
void Dialog::triActionNewDirSlot()
{
    ...
    m_pFtp->mkdir(stringToFtp(item->text(0)));
